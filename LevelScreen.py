...
class LevelScreen(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.return_button = Button(50, 50, 60, 60, 'return_button')
        ...
        self.background.blit(self.text, self.text_rect)
    ...
    def draw(self, window_screen):
        if self.state:
            if self.game_play == None:
                for level in LEVELS:
                    level_button = LevelButton(level["id"])
                    level_button.draw(self.background)
                    if click(level_button.rect):
                        pygame.mixer.music.stop()
                        self.game_play = Level(level)

                window_screen.blit(self.background, self.rect)
